...
class Quickturn():
    ...
    def wavedash(self) -> None:
        ...
        if t_ms - self.last_wavedash_time_ms >= self._wavedash_cooldown:
            ...
            if turn_power < 0.55:
                turn_power = 5 * turn_power * turn_power * turn_power * turn_power
            else:
                turn_power = 0.55 - pow(-2 * turn_power + 2, 4) / 2
            if turn_power < 0.1: return

            boost_power = math.sqrt(math.pow(vel[0],2) + math.pow(vel[1],2)) * 5
            ...
            ba.emitfx(position=self.node.position,
                      ...
                      chunk_type='spark',
                      count=5,
                      scale=boost_power * 1.5 / 160 * turn_power,
                      spread=0.25);

            # Play a sound
            bastd.actor.spaz.SpazFactory.get().quickturn_sound.play(0.6 + 0.4 * turn_power, position=self.node.position)
